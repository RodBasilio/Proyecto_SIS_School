...
$comments=isset($_POST["comments"])? limpiarCadena($_POST["comments"]):"";
$user_id=$_SESSION["idusuario"];

switch ($_GET["op"]) {
	case 'guardaryeditar':
	if (empty($id)) {
		$rspta=$conducta->insertar($kind_id,$date_at,$alumn_id,$team_id,$comments); 
		echo $rspta ? "Datos registrados correctamente" : "No se pudo registrar los datos";
	}else{
         $rspta=$conducta->editar($id,$kind_id,$date_at,$alumn_id,$team_id,$comments);
    ...
    case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
            ...
            //"5"=>'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->id.')"><i class="fa fa-pencil"></i></button>'
				);
		}
		$results=array(
